...
async function handleEmployeeImport(req, res, url) {
  ...
  try {
    ...
    if (commitMatch && method === "POST") {
      const body = await readJsonBody(req);
      const summary = await commitBatch(commitMatch[1], {
      ...
      const rows = await getBatchRows(commitMatch[1], true);
      createAccessLog({
        username: auth.user.username,
        userId: auth.user.id,
        role: auth.user.role,
        success: summary.failedOnCommit === 0,
        status: summary.failedOnCommit ? "failed" : "success",
        reason: `employee_import_commit batch=${commitMatch[1]} imported=${summary.importedRows || 0} failed=${summary.failedOnCommit || 0} pending=${summary.errorRows + summary.needsReviewRows}`,
        ip: req.socket?.remoteAddress || "",
        userAgent: req.headers["user-agent"] || "",
      });
      sendJson(res, 200, { ok: true, summary, rows });
    ...
    sendJson(res, 400, { ok: false, message: error.message || "Error en importacion de empleados." });
    return true;
  }
}
